...
class ArenaEngine
{
  ...
  public function shortcode_HONstats($atts)
  {
    ...
    while ($loop->have_posts()) : $loop->the_post();
      ...
      $return .= '<h3 class="wp-showdown-title"><a href="' . get_permalink() . '" title="' . get_the_title() . '" rel="bookmark">' . get_the_title() . '</a></h3>';
      ...
      $return .= '<p>Points: ' . (get_post_meta(get_the_ID(), 'points', true) + 0) . '</p>';
  ...
  public function shortcode_hotornot($atts)
  {
    ...
    $IPrestrictHON = "activate"; //$arenaOptions['IPrestrictHON'];
    $showTwitter = false;
...
<?php
    ...
    if (isset($_POST['whoshot'])) {
      ...
      $whoshot = intval($_POST['whoshot']);
      $newpoints = intval($_POST['arenawinner']);
      $lastIP = $_SERVER['REMOTE_ADDR'];



      if ($IPrestrictHON === "activate2") {
      ...
      if ($newpoints > 0) {
        ...
        $shown = intval(get_post_meta($whoshot, 'shown', true));
        $shown = $shown + 1;


        update_post_meta($whoshot, 'shown', $shown);

        $points = intval(get_post_meta($whoshot, 'points', true));
    ...
    while ($loop->have_posts()) : $loop->the_post();
      ...
      if ($image === "true") { // if image is selected - show featured image for competitor              
        ...
        //     $return .= get_the_post_thumbnail(get_the_ID(), 'large');
        $return .= '</div>';
      }

      $return .= '<p class="wp-showdown-voteherecaption">' . get_the_title() . '</p>';
